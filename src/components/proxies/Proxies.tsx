...
function Proxies({
  ...
  const [isProxyManagerModalOpen, setIsProxyManagerModalOpen] = useState(false);
  
  const closeSettingsModal = useCallback(() => {
    setIsSettingsModalOpen(false);
  }, []);
  
  const closeProxyManagerModal = useCallback(() => {
    setIsProxyManagerModalOpen(false);
  }, []);
  ...
  return (
    <>
      ...
      <BaseModal isOpen={isProxyManagerModalOpen} onRequestClose={closeProxyManagerModal}>
        <ProxyManager />
      </BaseModal>
      <div className={s0.topBar}>
        ...
        <div className={s0.topBarRight}>
          ...
          <Tooltip label={t('add_proxy')}>
            <Button kind="minimal" onClick={() => setIsProxyManagerModalOpen(true)}>
              <Plus size={16} />
            </Button>
          </Tooltip>
