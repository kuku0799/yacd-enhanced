...
# 在线部署脚本 - 可以直接从网络下载并执行
# 使用方法: curl -sSL https://raw.githubusercontent.com/kuku0799/yacd-enhanced/dist/deploy-online.sh | bash
...
if [ -f /etc/openwrt_release ]; then
    echo "[INFO] 检测到 OpenWrt/LEDE 系统"
else
    echo "[WARNING] 未检测到 OpenWrt 系统，但脚本将继续执行"
fi

# 安装必要工具
echo "[STEP] 检查并安装必要的工具..."
...
# 验证部署
echo "[STEP] 验证部署..."
required_files=("index.html" "assets" "manifest.webmanifest" "sw.js")
missing_files=()
...
if [ ${#missing_files[@]} -eq 0 ]; then
    ...
    echo "[ERROR] 部署验证失败，缺少文件: ${missing_files[*]}"
    exit 1
fi
